...
public class profesor extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String RequestURL = "http://"+ request.getServerName() + ":9090/CentroEducativo/profesores/" + dni + "?key=" + key;
		...
		RequestURL = "http://"+ request.getServerName() + ":9090/CentroEducativo/profesores/" + dni + "/asignaturas?key=" + key;
        ...
        out.println("<div class=\"container\">\n"
        		+ "                <div class=\"jumbotron\">\n"
        		+ "                  <h1>Estas son tus asignaturas</h1>\n"
		...
		try {
			...
			for(int i=0; i<jsonAsig.length(); i++) {
				String acronimo=jsonAsig.getJSONObject(i).getString("acronimo");
				String nombreAsig=jsonAsig.getJSONObject(i).getString("nombre");
				String curso=jsonAsig.getJSONObject(i).get("curso").toString();
				String cuatrimestre=jsonAsig.getJSONObject(i).getString("cuatrimestre");
				String creditos=jsonAsig.getJSONObject(i).get("creditos").toString();
				out.println("<p><a style=\"font-size: large;\"href=\"listaAlumnosAsignatura/"+ acronimo + "\" class=\"text-decoration-none\"\">" + nombreAsig + "</a></p>");
			}
		}catch (JSONException e) {
				e.printStackTrace();
		}
