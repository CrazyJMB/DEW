import java.io.*;
import java.util.Enumeration;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

/**
 * Servlet implementation class log1
 */
public class log1 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public log1() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Archivo donde guardar los datos
		File file = new File("./eclipse-workspace/version1/src/main/archivos/file1.txt");
		PrintWriter fileOut = new PrintWriter(file);
		
		response.setContentType("text/html");
		PrintWriter out = response.getWriter();
		
		out.println("<html><head><title>Info</title></head><body><table>");
		fileOut.write(" Info \n");
		out.println("<tr><td colspan=2><b>Info Path</b></td><tr>");
		out.println("<tr><td>Request URL</td><td>"+request.getRequestURL()+"</td><tr>");
		fileOut.write("Request URL: " + request.getRequestURL() + "\n");
		out.println("<tr><td>Context Path</td><td>"+request.getContextPath()+"</td><tr>");
		fileOut.write("Context Path: " + request.getContextPath() + "\n");
		out.println("<tr><td>Servlet Path</td><td>"+request.getServletPath()+"</td><tr>");
		fileOut.write("Servlet Path: " + request.getServletPath() + "\n");
		out.println("<tr><td>Path Info</td><td>"+request.getPathInfo()+"</td><tr>");
		fileOut.write("Path Info: " + request.getRequestURL() + "\n");
		out.println("<tr><td>Query String</td><td>"+request.getQueryString()+"</td><tr>");
		fileOut.write("Query String: " + request.getQueryString() + "\n");
		out.println("<tr><td colspan=2><b>Protocolo HTTP</b></td><tr>");
		fileOut.write("\n Protocolo HTTP \n");
		out.println("<tr><td>Metodo</td><td>"+request.getMethod()+"</td><tr>");
		fileOut.write("Metodo: " + request.getMethod() + "\n");
		out.println("<tr><td>Remote Addr</td><td>"+request.getRemoteAddr()+"</td><tr>");
		fileOut.write("Remote Addr: " + request.getRemoteAddr() + "\n");
		out.println("<tr><td>Remote Host</td><td>"+request.getRemoteHost()+"</td><tr>");
		fileOut.write("Remote Host: " + request.getRemoteHost() + "\n");
		out.println("<tr><td>Remote Port</td><td>"+request.getRemotePort()+"</td><tr>");
		fileOut.write("Remote Port: " + request.getRequestURL() + "\n");
		out.println("<tr><td colspan=2><b>Cabeceras HTTP</b></td><tr>");
		fileOut.write("\n Cabeceras HTTP: \n");
		Enumeration<String> names = request.getHeaderNames();
		while (names.hasMoreElements()) {
			String name = names.nextElement();
			out.println("<tr><td>"+name+"</td><td>"+request.getHeader(name)+"</td><tr>");
			fileOut.write(name + " " + request.getHeader(name) + "\n");
		}
		out.println("<tr><td colspan=2><b>Parametros</b></td><tr>");
		fileOut.write("\n Parametros \n");
		names = request.getParameterNames();
		
		while (names.hasMoreElements()) {
			String name = names.nextElement();
			out.println("<tr><td>"+name+"</td><td>"+request.getParameter(name)+"</td><tr>");
			fileOut.write(name + " " + request.getParameter(name) + "\n");
		}
		out.println("</table></body></html>");
		
		// Guarda de datos en un archivo html
		fileOut.close();		
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		doGet(request, response);
	}

}